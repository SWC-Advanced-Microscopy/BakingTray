function tileAcqDone(obj,~,~)
    ...
    if obj.parent.importLastFrames
        ...
        for z = 1 : obj.hC.hDisplay.displayRollingAverageFactor : length(obj.hC.hDisplay.stripeDataBuffer)
            ...
            elseif isempty(lastStripe.roiData)
                msg = sprintf('obj.hC.hDisplay.stripeDataBuffer{%d}.roiData is empty."',z);
            elseif ~iscell(lastStripe.roiData) && ~isempty(iscell(lastStripe.roiData))
            ...
            if ~isempty(msg) %if true there was an error
                msg = [msg, ' -- Not extracting tile data for preview image in SIBT.tileAcqDone'];
