classdef acquisition_view < BakingTray.gui.child_view
    ...
    methods(Hidden)
        ...
        function placeNewTilesInPreviewData(obj,~,~)
            ...
            % the obj.previewImageData array. This is run when obj.model.currentTilePosition
            % increments (which is done by SIBT.tileAcqDone). This method only runs once 
            % per X/Y position. 
            ...
            if obj.model.lastTilePos.X>0 && obj.model.lastTilePos.Y>0
                ...
                % If this is the last position and we are using ScanImage
                % pull tiles out of buffer manually and re-run. Otherwise
                % just run the updateSectionImage method again.
                if n==length(obj.model.positionArray)
                    if isa(obj.model.scanner,'SIBT')

                        %Pixel coords within preview image for last tile
                        y = (1:size(obj.model.downSampledTileBuffer,1)) + obj.previewTilePositions(n,2);
                        x = (1:size(obj.model.downSampledTileBuffer,2)) + obj.previewTilePositions(n,1);

                        obj.model.scanner.getTileDataFromScanImage;
