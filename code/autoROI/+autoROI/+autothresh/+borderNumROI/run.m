function [tThreshSD,stats,tThresh] = run(pStack, runSeries, settings, BBstats)
    ...
    %
    % function [tThreshSD,stats,tThresh] = autoROI.autoThresh.borderNumROI.run(pStack, runSeries, settings, BBstats)
    %
    % Purpose
    ...
    stats=calcStatsFromThreshold(minThresh); % Local nested function
    ...
    if runSeries
        ...
        autoROI.autothresh.borderNumROI.plot(stats)
        tThreshSD = nan;
        fprintf('Finished!\n')
        toc(t)
        return
